...
def PredictionfromQChem(QChem):
	...
	''' 0.2. Read the Coordinate and MO Coefficients of Reference
	   =============================='''
	Ref = ase.io.read('Nap_sym.SO.xyz')
	RefCoor = Ref.get_positions().reshape(1,-1)
	RefMO = np.fromfile('Nap_sym.SO.out.MO')
	...
	homo_prediction_model = joblib.load('NaphthaleneMonomer_Cart-difftoMO_15k_KRR.joblib')
	...
	kr=joblib.load('NaphthaleneDimer_CM_inter_OverlaptoCoupling_20k_KRR.joblib')
	ML_EC=kr.predict(Feature)
	return ML_EC[0]

if __name__ == '__main__':
	ML_EC = PredictionfromQChem(sys.argv[1])
	#ML_EC = Parallel(n_jobs=4)(delayed(PredictionfromQChem)('Add_'+str(i).zfill(3)+'.inp') for i in range(1,20))
	print(ML_EC)
